...
st.title("Yearly Growth Rate Calculator")

st.write("""
This dashboard calculates the per annum growth required to reach a target GDP value from India's current GDP over a specified number of years.
""")
...
current_billion = st.number_input("Current GDP of India (Billion $)", min_value=0.0, value=current_gdp/1e9, step=1.0, format="%.2f")
target_billion = st.number_input("Target GDP of India (Billion $)", min_value=0.0, value=30000.0, step=1.0, format="%.2f")
...
if current > 0 and target > 0 and time > 0:
    try:
        ...
        with col1:
            ...
            st.markdown(f"<span style='font-size:2em;'><b>{growth:.2f}%</b></span>", unsafe_allow_html=True)
        with col2:
            if latest_growth is not None:
                color = 'green' if latest_growth >= growth else 'red'
                st.markdown(f"<b>Latest GDP Growth Rate ({latest_year})</b>", unsafe_allow_html=True)
                st.markdown(f"<span style='font-size:2em; color:{color};'><b>{latest_growth:.2f}%</b></span>", unsafe_allow_html=True)
            else:
                st.markdown("<b>Latest GDP Growth Rate</b>", unsafe_allow_html=True)
                st.markdown(f"<span style='font-size:2em; color:gray;'><b>N/A</b></span>", unsafe_allow_html=True)
        ...
        if india_pop:
            ...
            col1, col2 = st.columns(2)
            with col1:
                st.markdown("<b>Current Per Capita GDP (India, {year}):</b>".format(year=india_pop_year), unsafe_allow_html=True)
                st.markdown(f"<div style='font-size:2.5em; font-weight:bold; color:#1f77b4;'>$ {current_per_capita:,.2f}</div>", unsafe_allow_html=True)
            with col2:
                st.markdown(f"<b>Projected Per Capita GDP (India, {target_year}):</b>", unsafe_allow_html=True)
                st.markdown(f"<div style='font-size:2.5em; font-weight:bold; color:#2ca02c;'>$ {projected_per_capita:,.2f}</div>", unsafe_allow_html=True)
            ...
            try:
                ...
                df['GDPPerCapita_GDPPerCapitaViaIMF_usd_2025'] = df['GDPPerCapita_GDPPerCapitaViaIMF_usd_2025'].replace('[\$,]', '', regex=True).astype(float)
                closest_5 = df.iloc[(df['GDPPerCapita_GDPPerCapitaViaIMF_usd_2025'] - projected_per_capita).abs().argsort()[:5]]
                st.markdown(f"<br/><b>5 Countries with most similar per capita GDP to India's projected ({target_year}):</b>", unsafe_allow_html=True)
    ...
    st.warning("Please enter positive values for all inputs and ensure target year is in the future.") 